...
task runr, "Runs connect4 for current platform":
 exec &"nim c -r {releaseOpts} -o:connect4 src/main.nim"

task rund, "Runs debug connect4 for current platform":
 exec &"nim c -r {debugOpts} -o:connect4 src/main.nim"

task release, "Builds connect4 for current platform":
 exec &"nim c {releaseOpts} -o:connect4 src/main.nim"

task webd, "Builds debug connect4 for web":
 exec &"nim c {debugOpts} -d:emscripten -o:connect4.html src/main.nim"

task webr, "Builds release connect4 for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:connect4.html src/main.nim"

task debug, "Builds debug connect4 for current platform":
 exec &"nim c {debugOpts} -o:connect4_debug src/main.nim"
